...
function storeQueryResults(results, query) {
  ...
  for (i = 0; i < results.length; i++) {
    ...
    if (includes.length) {
      var flattenAndStoreValue = function flattenAndStoreValue(val) {
        return storeObject(flatten(val));
      };
...
function deepFetch(id, seen) {
  ...
  var obj = {};
  var seenChildren = [];
  var populatePointer = function populatePointer(val) {
...
var Subscription = (function () {
  ...
  _createClass(Subscription, [{
    ...
    value: function issueQuery() {
      ...
      if (this.originalQuery._observeCount) {
        this.originalQuery.count().then(function (result) {
          _this.pending = false;
          _this.resultCount = result;
          _this._forEachSubscriber(function (subscriber) {
